...
class ReportGenerator:
    ...
    def _generate_recommendations_section(self) -> str:
        ...
        for _, row in self.risk_df.iterrows():
            ...
            if recs is not None:
                # Handle different stored formats: list/tuple/ndarray or semicolon-delimited string
                if isinstance(recs, str):
                    # Parse recommendations if stored as a delimited string
                    html += "<ul>"
                    for rec in recs.split(';'):
                        if rec and str(rec).strip():
                            html += f"<li>{rec.strip()}</li>"
                    html += "</ul>"
                elif isinstance(recs, (list, tuple, np.ndarray)):
                    html += "<ul>"
                    for rec in recs:
                    ...
                    # Fallback for other types (e.g., single value)
                    html += f"<p>{recs}</p>"
