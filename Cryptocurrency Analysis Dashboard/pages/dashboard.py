...
with st.container():
    toolbar_col1, toolbar_col2, toolbar_col3 = st.columns([3, 2, 5])
    ...
    with toolbar_col3:
        ...
        try:
            ...
            crypto_list = list(prices.items())
            for crypto, price in crypto_list:
                st.metric(
                    f"{get_crypto_emoji(crypto)} {crypto.capitalize()}",
                    MetricsFormatter.format_price(price)
                )
        except Exception:
...
with tab1:
    st.subheader(f"{selected_crypto.capitalize()} Price and Volume")
...
with tab3:
    st.subheader("Risk-Return Profile (All Assets)")
    if len(metrics_df) > 0:
...
with tab5:
    st.subheader("🛡️ Risk Assessment & Classification")
